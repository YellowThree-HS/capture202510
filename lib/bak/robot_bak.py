...
import re
import lib.dobot_api as DobotApi
import time
...
class Robot:
    ...
    def move_to(self, x, y, z, rx, ry, rz, mode='joint'):
        """
        移动机器人到指定位置。
        ...
        print(f"准备以 {mode} 模式移动到: ({x}, {y}, {z}, {rx}, {ry}, {rz})")
        try:
            if mode.lower() == 'joint':
                # MovJ 需要坐标模式参数，1表示笛卡尔坐标系
                result = self.dashboard.MovJ(x, y, z, rx, ry, rz, 0)
                print(f"MovJ 返回: {result}")
            elif mode.lower() == 'linear':
                # MovL 需要坐标模式参数，1表示笛卡尔坐标系
                result = self.dashboard.MovL(x, y, z, rx, ry, rz, 0)
