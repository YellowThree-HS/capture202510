...
class DobotRobot(Robot):
    ...
    def moveL(self, pose: np.ndarray) -> None:
        """Command the leader robot to a given xyzrxryrz state.

        Args:
            pose (np.ndarray): The xyzrxryrz state to command the leader robot to.
        """
        assert not self.robot_is_err, f"{self.robot_ip}: error!"
        tic = time.time()
