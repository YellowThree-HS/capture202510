...
def main(use_camera=True):
    ...
    # ==================== 键盘控制参数 ====================
    # 移动步长（毫米和度）
    pos_step = 10.0   # 位置步长 10mm
    rot_step = 5.0    # 旋转步长 5度
            ...
            if marker_ids is not None and len(marker_ids) > 0:
                ...
                if charuco_corners is not None and len(charuco_corners) > 4:
                    ...
                    retval, rvec, tvec = cv2.solvePnP(
                        obj_points, charuco_corners, 
                        camera_matrix, distortion_coefficients
                    )
                    
                    if rvec is not None and tvec is not None:
                        cv2.drawFrameAxes(image_copy, camera_matrix, 
                                        distortion_coefficients, rvec, tvec, length=0.03)
                        # 显示检测到的角点数
                        cv2.putText(image_copy, f"Corners: {len(charuco_corners)}", 
                                  (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            # 显示模式信息
            mode_text = "Fine Mode" if fine_mode else "Coarse Mode"
            step_text = f"Step: {pos_step_fine if fine_mode else pos_step}mm / {rot_step_fine if fine_mode else rot_step}°"
            cv2.putText(image_copy, mode_text, (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            cv2.putText(image_copy, step_text, (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            cv2.putText(image_copy, f"Data: {len(t_end2base)}/15", (10, 120), 
