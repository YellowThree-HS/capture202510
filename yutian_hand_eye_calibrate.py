...
def camera_calibrate(iamges_path):
    ...
    obj_points = []  # 存储3D点
    img_points = []  # 存储2D点
    img_files = [f for f in os.listdir(iamges_path) if f.lower().endswith(('.jpg', '.png'))]
    img_files_num = len(img_files)
    for i in range(0, img_files_num):  # 标定好的图片在iamges_path路径下，从0.jpg到x.jpg   一般采集20张左右就够，实际情况可修改
        ...
        if os.path.exists(image):
            ...
            cv2.waitKey(20)  ## 停留1s, 观察找到的角点是否正确
